...
def home_view(request):
    ...
    if request.user.is_authenticated:
        cars=models.Cars.objects.all()
        return HttpResponseRedirect('afterlogin',{'cars':cars})
    return render(request,'vehicle/index.html',{'cars':cars})

def relog_view(request):
    return render(request,'vehicle/relog.html')
...
def adminclick_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return HttpResponseRedirect('royalogin')
...
@login_required(login_url='royalogin')
def admin_dashboard_view(request):
    ...
    'total_cars':models.Cars.objects.all().count(),
...
def admin_cars(request):
    return render(request,'vehicle/admin_cars.html')
...
@login_required(login_url='royalogin')
def admin_view_cars_view(request):
    cars=models.Cars.objects.all()
    return render(request,'vehicle/admin_view_cars.html',{'cars':cars})
...
@login_required(login_url='royalogin')
def admin_add_cars_view(request):
    carsForm=forms.CarsForm()
    mydict={'carsForm':carsForm}
    if request.method=='POST':
        carsForm=forms.CarsForm(request.POST,request.FILES)
        if carsForm.is_valid():
            car=carsForm.save(commit=False)
            car.save()
        return HttpResponseRedirect('/admin-view-cars')
    return render(request,'vehicle/admin_add_cars.html',context=mydict)

@login_required(login_url='royalogin')
def update_cars_view(request,pk):
    car1=models.Cars.objects.get(id=pk)
    car=forms.CarsForm(request.POST,request.FILES,instance=car1)
    mydict={'car':car , 'car1':car1}
    if request.method=='POST':
        carsForm=forms.CarsForm(request.POST,request.FILES,instance=car1)
        if carsForm.is_valid():
            carsForm=carsForm.save()
            carsForm.save()
            return redirect('admin-view-cars')
    return render(request,'vehicle/update_cars.html',context=mydict)


@login_required(login_url='royalogin')
def delete_cars_view(request,pk):
    car=models.Cars.objects.get(id=pk)
    car.delete()
    return redirect('admin-view-cars')
